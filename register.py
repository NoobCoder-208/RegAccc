...
def tudongcapnhat(region="VN"):
 ...
 try:
  res=requests.get(I % (ver, region.upper()))
  return res.json()
 except:
...
async def OnTcp(ip,port,auth_token_hex):
 try:
...
def guest_register():
 ...
 if resp.status_code!=200: raise Exception(f"Register failed: HTTP {resp.status_code}")
 data=resp.json()
...
def guest_token(uid,password):
 ...
 if resp.status_code!=200: raise Exception(f"Token failed: HTTP {resp.status_code}")
 data=resp.json()
